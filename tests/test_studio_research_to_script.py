...
class FakeLLM:
    def __init__(self, *, unknown_url: bool = False, bad_proof: str | None = None) -> None:
...
def _workflow(*, unknown_url: bool = False, bad_proof: str | None = None) -> ResearchToScriptWorkflow:
    return ResearchToScriptWorkflow(search_provider=FakeSearch(), extractor=FakeExtractor(), llm=FakeLLM(unknown_url=unknown_url, bad_proof=bad_proof))
...
def test_research_to_script_rejects_claim_link_not_present_in_sources() -> None:
    with pytest.raises(ResearchToScriptError, match="absent from extracted research"):
