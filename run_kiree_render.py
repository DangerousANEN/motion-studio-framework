...
def main():
    ...
    audio_dir = OUT_DIR / "audio_kiree"
    audio_dir.mkdir(exist_ok=True)

    st = {"storyboard": STORYBOARD, "preset":"HeroKinetic", "accent":"gold", "agent_level":5, "video_format":"vertical"}
    st = node_script_split(st)

    ref_audio, ref_text = resolve_voice("kireevoice")
    print(f"[kireevoice] Synthesizing audio for {len(st['scenes'])} scenes...", flush=True)
    ...
    raw_mp4 = str(OUT_DIR / "v4_kireevoice_raw.mp4")
    final_mp4 = str(OUT_DIR / "v4_kireevoice.mp4")

    print("[render] Starting Remotion render with kireevoice...", flush=True)
    render_remotion_video(spec, raw_mp4)
    print(f"[render] Raw MP4 created: {raw_mp4}", flush=True)

    audio_list_path = OUT_DIR / "audio_kiree_list.txt"
    with open(audio_list_path, "w") as f:
        for fpath in audio_files:
            fpath_clean = str(fpath).replace("\\", "/")
            f.write(f"file '{fpath_clean}'\n")

    merged_wav = str(OUT_DIR / "merged_kiree_audio.wav")
